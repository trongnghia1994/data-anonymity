...
from oka_py3.utils.utility import get_num_list_from_str, cmp_str, qid_to_key
...
def find_best_knn(index, k, data):
    ...
    for i, t in enumerate(data):
        ...
        dist_dict[i] = dist
    sorted_dict = sorted(iter(dist_dict.items()), key=operator.itemgetter(1))
    knn = sorted_dict[:k - 1]
    knn.append((index, 0))
...
def clustering_oka(data, k=25):
    ...
    less_clusters = []
    # randomly choose seed and find k-1 nearest records to form cluster with size k
    seed_index = random.sample(list(range(len(data))), int(len(data) / k))
